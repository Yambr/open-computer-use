...
- **System prompt endpoint** — `/system` returns environment-aware prompt for LLM grounding. Open Computer Use does the same via `/system-prompt` with auto-injection of skills, tool descriptions, and per-user configuration — so this is a shared feature, not a differentiator
