...
# Phase 2 — Plan 02-02 removes the deprecation step).
#
# Invokes in sequence:
...
echo "[1/5] Per-CLI docstring surface (test-subagent-cli-surface.py)"
run_pytest "test-subagent-cli-surface.py" "$TEST_DIR/test-subagent-cli-surface.py"

echo ""
echo "[2/5] opencode error event mapping (test-opencode-error-mapping.py)"
run_pytest "test-opencode-error-mapping.py" "$TEST_DIR/test-opencode-error-mapping.py"

echo ""
echo "[3/5] Sub-agent docstring helper unit tests (test_subagent_docstring.py)"
run_pytest "test_subagent_docstring.py" "$TEST_DIR/test_subagent_docstring.py"

echo ""
echo "[4/5] Default model resolution order (test-default-model-resolution.py)"
run_pytest "test-default-model-resolution.py" "$TEST_DIR/test-default-model-resolution.py"

echo ""
echo "[5/5] list-subagent-models script invocation (test-list-subagent-models.sh)"
